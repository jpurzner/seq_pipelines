...
# pipeline for processing ChIP-seq data, which has been modified to process 
# DNAase hypersensitivity data, simply crops the reads to 20 BP 
...
def genome_dict(machine = "scg3",  genome = 'mm9'): 
    ...
    if machine == 'scg3' and genome == 'mm9':
        genome_file = {
           ...
           'mappability': '/srv/gsfs0/projects/cho/genomes/mm9/mappability/globalmap_k2tok54'    
        }
        ...
        genome_file = {
            ...
            'mappability': '/tank/genomes/align2raw/umap/globalmap_k2tok54'
        }
    else: 
    ...
    if pairedEnd:
        ...
        if isinstance(tempFastqFile, list): # fastqFile is a list          
            for i, fastq in enumerate(tempFastqFile):
                cmd += "java -Xmx6g -Xms3g -Djava.awt.headless=true -jar %s SE -phred33 %s %s LEADING:2 TRAILING:2 CROP:20 >> %s 2>&1\n" % (trimmomaticString, fastq, tempFastqFileTrim[i], trimlogFile)
        else:
            cmd += "java -Xmx6g -Xms3g -Djava.awt.headless=true -jar %s SE -phred33 %s %s LEADING:2 TRAILING:2 CROP:20 >> %s 2>&1\n" % (trimmomaticString, fastq, tempFastqFileTrim, trimlogFile)
...
def bam2tagAlign(samtoolsString, fileNameDict):
    ...
    # modified to prevent screening reads by tagsize, size is restricted at the point of filtering
    cmd  = "%s view -F 0x0204 %s | gawk 'BEGIN{OFS=\"\\t\"}{if (and($2,16) > 0) {print $3,($4-1),($4-1+length($10)),\"N\",\"1000\",\"-\"} else {print $3,($4-1),($4-1+length($10)),\"N\",\"1000\",\"+\"} }' > %s" % (samtoolsString, tempRmdupBamFile, temptagAlignFile)
